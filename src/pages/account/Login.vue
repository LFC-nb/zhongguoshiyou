        ...
        <div class="hdp-uf hdp-uf-vc hdp-uf-dc">
          ...
          <div class="form">
            <div class="logo-content hdp-uf hdp-uf-hc">
              <p>
                <img
                  src="../../assets/images/anyue-logo-black.png"
                  alt="logo"
                />
              </p>
            </div>
            ...
            <el-form-item label="验证码" prop="vaildCode" style="width: 400px">
              <el-row>
                ...
                <el-col :span="8" :offset="2">
                  <button class="code-btn" :disabled="btnDisabled">
                    {{ codeText }}
                  </button>
                </el-col>
              </el-row>
...
export default {
  ...
  methods: {
    init() {
      ...
      this.$store.dispatch(
        "menuLeadLists/setMenuLeadListsData",
        this.currentMenuLeadListsData
      );
    },
    async login() {
      ...
      localStorage.removeItem("NoShowRightinclud91");
      localStorage.clear();
      let resData = await demoApi.getLoginData(conditionsParams);
      ...
      if (resData.status === "000") {
        ...
        if (!commonFunc.isDefine(resData.data.record.Module)) {
          this.$message.error("该用户未分配管理权限");
          return;
        }
        ...
        //   code: '91',
        //   icon: '',
        //   link: '/vehicleManagement/truckapply',
        //   name: '用车申请'
        // }
        // mockData.userInfo[0].menuLists[0].children.push(ww)
        // console.log(resData.data.record.Token)
        ...
        localStorage.setItem(
          "userDataInfo",
          JSON.stringify(resData.data.record)
        );
        localStorage.setItem("isLogin", true);
        localStorage.setItem("issupplierLogin", false); // 标识区分后勤人员还是供应商登录 用于点击退出时判断回到那个登录页面
        localStorage.setItem(
          "NoShowRightinclud91",
          resData.data.record.NoShowRight.includes(91)
        );
        localStorage.setItem(
          "CarInputRight",
          resData.data.record.CarInputRight
        );
        this.getAllbaseData();
        this.gotofirstroute();
      } else if (resData.status === "3009") {
        this.$message.error("该用户未分配管理权限");
      } else {
    ...
    gotofirstroute() {
      this.$router.push({
        ...
        query: {
          ...
          code: mockData.userInfo[0].menuLists[0].children[0].code,
    ...
    async getdepartmentlist() {
      ...
      if (resData.status === "000") {
        ...
        localStorage.setItem(
          "departmentcascader",
          JSON.stringify(mockData.cascader)
        );
        // commonFunc.myConsole(resData)
